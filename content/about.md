---
title: "About"
date: 2024-01-01
showAuthorBottom: true
---

I am a software developer with a Master's Degree in Computer Engineering from Politecnico di Torino (Italy).
My journey spans from early web experiments in 2006 to professional work at companies like Amadeus, Telemaque, and WildMoka.

I have a broad skill set — from low-level C++ and bioinformatics tools, to Android apps, full-stack web development with Python and Node.js, and AI voice skills with Alexa.

---

## Technical Skills

**Languages:** Python, Node.js, JavaScript, C++, Java, Bash, R, SQL  
**Tools & platforms:** Android, Docker, Git, Linux

---

## Background

{{< accordion >}}
{{< accordionItem header="Experience" md=false >}}

{{< timeline >}}
{{< timelineItem md=true icon="briefcase" header="Fullstack Developer @ WildMoka" badge="Feb 2020 - Present" >}}
Fullstack developer on the WildMoka Platform for automated video production and live streaming.
- Backend services with Python and Node.js
- Frontend features in JavaScript
- Platform integrations and API development
{{< /timelineItem >}}

{{< timelineItem md=true icon="briefcase" header="Android Developer @ Telemaque" badge="Sep 2019 - Dec 2019" >}}
Developed Android applications for an enterprise mobile product.
- Native Android development (Java/Kotlin)
- Integration with REST APIs
{{< /timelineItem >}}

{{< timelineItem md=true icon="briefcase" header="Software Developer @ Amadeus IT Group" badge="Jan 2018 - Aug 2019" >}}
Development of the Rail ticketing platform.
- C++ backend development for high-performance reservation systems
- Python scripting and tooling
- Collaborative Agile environment with international teams
{{< /timelineItem >}}

{{< timelineItem md=true icon="briefcase" header="Bioinformatics Research Fellow" badge="Feb - Dec 2017" >}}
Conducted research in bioinformatics at **Politecnico di Torino**, producing two published tools:
- **[GeneEx](http://www.scitepress.org/PublicationsDetail.aspx?ID=ZaT7yr1Kba8=&t=1)** — Differential expression analysis tool — R
- **[BioSeqZip](https://pubmed.ncbi.nlm.nih.gov/31999333/)** — NGS redundant reads collapser — C++
- Coursera Machine Learning Certificate (2017)
- Participated in Droidcon Hackathon 2017
{{< /timelineItem >}}
{{< /timeline >}}
{{< /accordionItem >}}

{{< accordionItem header="Education" md=false >}}

{{< timeline >}}
{{< timelineItem md=true icon="graduation-cap" header="Master's Degree in Computer Engineering" badge="2013 - 2016" >}}
**Politecnico di Torino**

Graduated with a focus on software systems and algorithms. Thesis in Bioinformatics: pairwise alignment scoring analysis.
{{< /timelineItem >}}

{{< timelineItem md=true icon="graduation-cap" header="Bachelor's Degree in Computer Engineering" badge="2009 - 2013" >}}
**Università degli Studi di Palermo**

Foundation in computer science, algorithms, databases, and software engineering. Early web and mobile application experiments during studies.
{{< /timelineItem >}}
{{< /timeline >}}
{{< /accordionItem >}}
{{< /accordion >}}

---

Feel free to browse my [Projects](/projects/), take a look at my code on [GitHub](https://github.com/skiwithuge), or reach out on [LinkedIn](https://www.linkedin.com/in/orazio-scicolone-16235169/).
